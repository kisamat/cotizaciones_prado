...
class Categmis extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('admin/Categmis_model');
        $this->lang->load('auth');
    }
    ...
    function index() {
        ...
        if (!$this->ion_auth->logged_in()) {
            ...
            $this->smarty1->assign("categorias", $this->Categmis_model->get_todos());
            $this->smarty1->assign("title", 'Categorias Cajas Misionales');
            $this->smarty1->assign("class", "fa fa-shield");
            //Se carga el Template Users
            $this->smarty1->view('admin/categmis/categmis');
        }
    }

    // create a new category
    function crear_categmis() {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $this->smarty1->assign("title", 'Crear Categoria Caja Misional');
            $this->smarty1->assign("class", "fa fa-shield");
            $this->smarty1->view('admin/categmis/crear_categmis');
        }
    }
    // edit a category
    function editar_categmis($id) {
        ...
        $categoria = $this->Categmis_model->get_by_id($id);
        if (empty($categoria)) {
            redirect('admin/categmis', 'refresh');
        }
        ...
        if (isset($_POST) && !empty($_POST)) {
            if ($this->form_validation->run() === TRUE) {
                $categoria_update = $this->Categmis_model->edit($id);
        ...
        $this->smarty1->assign("nombre", $this->data['nombre']);
        $this->smarty1->assign("button", $this->data['button']);
        $this->smarty1->assign("title", 'Editar Categoria Cajas Misionales');
        $this->smarty1->assign("class", "fa fa-shield");
        $this->smarty1->view('admin/categmis/editar_categmis');
    }

    // delete a category
    function borrar_categmis($id) {
        ...
        if ($this->input->post()) {
 		...
 		$categoria_delete = $this->Categmis_model->elim($id);
        ...
        $this->smarty1->assign("button", $this->data['button']);
        $this->smarty1->assign("title", 'Eliminar Categoria Caja Misional');
        $this->smarty1->assign("class", "fa fa-shield");
        $this->smarty1->view('admin/categmis/borrar_categmis');
    }
}
